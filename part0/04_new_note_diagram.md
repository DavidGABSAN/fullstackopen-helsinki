...
participant server
    ...
    activate server
    server-->>browser: 302 Found (Redirection to /exampleapp/notes specified in Location header)
    deactivate server
    ...
    Note right of browser: The browser executes the callback function that renders the notes. One of these notes is the new one added before.
```
